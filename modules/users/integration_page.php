	...
	if(!isset($URI[2])){
		...
		if(md5(strtolower($_POST['captcha']).$Config->ticketSalt) != $_COOKIE['captcha']){
			...
			if ($CUser->email == $_POST['email']){
				...
				addmail($CUser->email, "Ссылка для генерации нового пароля", $txt, $Contact->mail);
		...
		if(!$Config->registration){
			...
			$return.= '<p class="p_link_back"><a href="/'.$URI[1].'/reg">Вернуться назад</a></p>';
			System::notification('Ошибка при регистрации пользователя: ip '.IP.' заблокирован.', 'r');
		}elseif(filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) === false){
			$page->name = 'Ошибка';
			$return = '<p>Введенный емейл некорректен</p>
			<p class="p_link_back"><a href="/'.$URI[1].'/reg">Вернуться назад</a></p>';
			System::notification('Ошибка при регистрации пользователя: email введен некорректно.', 'r');
		}elseif(in_array(strtolower($_POST['email']), System::listEmailsUsers())){
			$page->name = 'Ошибка';
			$return = '<p>Пользователь с таким email уже существует</p>';
			$return.= '<p class="p_link_back"><a href="/'.$URI[1].'/reg">Вернуться назад</a></p>';
			System::notification('Ошибка при регистрации пользователя: введен email "'.$_POST['email'].'", который уже существует.', 'r');
		}elseif(System::isBadMailDomain($_POST['email']) && $Config->userEmailFilterList == 1){
			$page->name = 'Ошибка';
			$return = '<p>Такой email иметь запрещено</p>';
			$return.= '<p class="p_link_back"><a href="/'.$URI[1].'/reg">Вернуться назад</a></p>';
			System::notification('Ошибка при регистрации пользователя: введен email "'.$_POST['email'].'", который система определила как временный.', 'r');
		}elseif(!System::isGoodMailDomain($_POST['email']) && $Config->userEmailFilterList == 2){
			$page->name = 'Ошибка';
			$return = '<p>Такой email иметь запрещено</p>';
			$return.= '<p class="p_link_back"><a href="/'.$URI[1].'/reg">Вернуться назад</a></p>';
			System::notification('Ошибка при регистрации пользователя: введен email "'.$_POST['email'].'", который система определила как ненадежный.', 'r');
		}else{
			$salt = random(100);
		...
		if(($CUser = User::getConfig($URI[3])) !== false){
			if($CUser->newPasswordChecksum != '' && $CUser->newPasswordChecksum == $URI[4]){
				...
				$txt.= "Сайт: ".SERVER;
				addmail($CUser->email, "Ваш новый пароль", $txt, $Contact->mail);
				System::notification('На email пользователя '.$CUser->login.' отправлена ссылка для генерации нового пароля', 'g');
