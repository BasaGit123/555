...
setTimeout('window.location.href = \'iframefiles2.php?id=<?php echo $id;?>&dir=<?php echo $dir;?>\';', 200);
</script>
<?php
	}
	...
	if($act == 'index'){
		...
		echo'<div class="up">
			<form name="uploadform" enctype="multipart/form-data" action="iframefiles2.php?id='.$id.'" method="post">
				<INPUT TYPE="hidden" NAME="act" VALUE="upload">
				<INPUT TYPE="hidden" NAME="dir" VALUE="'.$dir.'">
				<INPUT class="btn btn-primary" TYPE="file" NAME="userfile"> <input class="btn btn-primary" type="submit" name="" value="Загрузить">
			</form>
		</div>
		<div class="directory">
		<span>Директория:</span> <a href="iframefiles2.php?id='.$id.'&amp;dir=..">'.SERVER.'</a> / ';
		$new_dir_url='..';
		...
		foreach($new_dir_url_arr as $value){
			...
			echo'<a href="iframefiles2.php?id='.$id.'&amp;dir='.$new_dir_url.'">'.$value.'</a> / ';
		}
		echo'</div>';
		...
		if(file_exists($dir))
		{
			if(is_dir($dir))
			{
				...
				foreach($param as $key => $value)
				{
					...
					{
						++$count;
						echo'<a href="iframefiles2.php?id='.$id.'&amp;dir='.$dir.'/'.$key.'"><img src="include/dir.png" alt=""><span>'.$key.'</span></a>';
					}
