...
var uploadform = '<form name="uploadform" enctype="multipart/form-data" action="modules.php?" method="post"><div class="a">'+
	...
	'<div class="btn-gr"><input class="btn btn-primary" type="submit" name="" value="Загрузить и установить"> <button class="btn btn-secondary" type="button" onclick="closewindow(\'window\');">Отмена</button>'+
	'</div></div></form>';
var init = '<div class="a">'+
	...
	'<div class="btn-gr"><button class="btn btn-primary" type="button" onClick="window.location.href = \'modules.php?act=init\';">Инициализировать</button> <button class="btn btn-secondary" type="button" onclick="closewindow(\'window\');">Отмена</button>'+
	'</div></div>';
var nodell = '<div class="a">'+
	'Это расширение нельзя удалять'+
	'<div class="btn-gr"><button class="btn btn-secondary" type="button" onclick="closewindow(\'window\');">Закрыть</button>'+
	'</div></div>';
function dell(m, n){
return '<div class="a">Подтвердите удаление расширения: <i>' + n + '</i>' +
	'<div class="btn-gr">' +
	'<button class="btn btn-danger" type="button" onClick="window.location.href = \'modules.php?act=dell&amp;module='+m+'&amp;titleshow=off\';">Удалить</button> '+
		...
		<div class="header">
				<div class="container">
					<div class="mobile-menu-wrapper">
						...
						<ul class="nav">
							<a href="module.php?module=news&amp;act=edit&amp;category=0&amp;nom_page=1">← Назад</a>
							
							<a class="nav-item" href="javascript:void(0);" onclick="openwindow(\'window\', 650, \'auto\', uploadform);">Загрузить и установить расширение</a>
							<a class="nav-item" href="javascript:void(0);" onclick="openwindow(\'window\', 650, \'auto\', init);">Инициализация расширений</a>
							<a class="nav-item" href="//my-engine.ru/extensions" target="_blank">Посмотреть каталог расширений на веб сайте разработчиков &#8594;</a>
			...
			if($_FILES['userfile']['error'] == '0'){
				...
				if(move_uploaded_file($_FILES['userfile']['tmp_name'], '../files/'.$file_name)){
					...
					if(class_exists('ZipArchive')){
						...
						if($zip->open('../files/'.$file_name) === TRUE){
							...
							while($name = $zip->getNameIndex($i)) {
								$dirindex[$i] = strstr($name, '/', true); // 5.1.41
								$i++;
							}
							$dirindex = array_unique($dirindex);
							$zip->close();
							$extracted = true;
						...
						if($list == 0){
							...
							$dirindex[0] = strstr($list[0]['stored_filename'], '/', true); // 5.1.41
							$extracted = true;
					...
					if($extracted){
						...
						foreach($dirindex as $value){
							$info = Module::info($value);
							System::notification('Установлено расширение '.$info['name'].'', 'g');
							
							if(Module::isIntegrationInstall($value)){
								require Module::pathRun($value, 'integration_install');	
							}
						}
